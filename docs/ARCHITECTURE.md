...
| `forge-procgen` | started (phase 2, 2026-09-26, on the cloud branch) | `field` (`Field2<T>`, a square grid with a spacing; sampling, Horn's gradient), `noise` (gradient noise on an integer lattice hashed with `pcg3d`, `fbm`, ridges: no transcendental, D-016), `island` (stages 1–2 of `docs/research/terrain-genesis.md`: the mask from a distance shaping warped by noise, the uplift, hardness and rain fields), `flow` (priority flood, D8 receivers with a pinned tie-break, the downstream-first stack, integer drainage areas), `erosion` (the implicit stream-power law with `n = 1` in stack order, hillslope diffusion, lakes as depressions that fill with sediment), `preview` (PNGs: height, hillshade, flow, an overview with sea, rivers and lakes); `tools/genesis` runs it (`docs/demos/island.md`). Planned: the basin graph in place of the flood per step, D∞ moisture, rivers as polylines and lakes by Fill–Spill–Merge, the ×2 amplification per tile, the material rule table, the hand-off to the cluster-DAG cook; later ecosystems, settlements, grammars |
