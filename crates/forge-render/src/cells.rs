...
//! what the GPU instance table and the frame block store. The camera-relative position of an
//! instance is its cell difference to the camera's, taken in integers (exact), times the cell
//! size (a power of two, so the product is exact), plus the difference of the two offsets: exact
//! near the camera wherever the scene stands in the world, so the table is never rewritten
//! when the camera moves and holds no `f64` (Freese 2004; `docs/research/large-worlds.md` §1).
//!
//! The renderer also keeps a **scene frame**: the frame of the top-level acceleration structure,
//! the probes and the dust's noise, anchored at the scene's origin ([`crate::MeshletScene::origin`]).
//! Rays start from `camera-relative position + Frame::camera_in_scene`, which the renderer
//! computes each frame with [`CellPos::relative_to`].
