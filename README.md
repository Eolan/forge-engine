...
shaders/              Slang sources (bindless, meshlet, barycentrics, vis64, hzb, starfield, atmosphere, atmosphere_luts, sky, sh, bloom, gtao, taa, exposure, tonemap, display, overlay, mipcheck)
...
ray-traced shadows (issue #45) and by the sky's light (issue #47), occluded by GTAO (issue #48). Keys: **L** / **K** LOD and its
colours, **M** cluster colours, **O** occlusion, **R** software rasteriser, **H** its pixels,
**[** / **]** LOD threshold, **T** TAA, **B** bloom, **J** shadows, **I** sky light, **N** ambient occlusion, **V** its view, **Tab** wireframe, **G** tone curve. Options:
...
`--no-taa`, `--no-shadows`, `--no-sky-light`, `--no-ao`, `--ao-radius M`, `--show-ao`, `--no-lod`, `--no-occlusion`, `--lod-error PX`,
`--sw-raster auto|on|off`, `--ev100 EV`, `--force-fallback`. At 1440p the flight at 300 m/s runs
at 2.46 ms of GPU with everything on. Numbers: [docs/demos/city-blocks.md](docs/demos/city-blocks.md).
