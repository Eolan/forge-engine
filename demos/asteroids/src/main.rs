...
impl Demo for Ballad {
    ...
    fn title(&mut self, _ctx: &Context) -> Option<String> {
        ...
        let title = format!(
            "forge asteroids | {} asteroids, {} meshes, {:.1} M meshlets, {:.0} M tris | {}: drawn {:.0} k + {:.0} k meshlets ({:.0} k in software, {:.2} M dense triangles, {:.1} k left to pass 2; {:.0} k work items, {:.0} k roots), {:.2} M tris | GPU {:.2} ms  CPU {:.2} ms  frame p50 {:.2} p99 {:.2} ms | EV100 {:.1} {} | {}{}{}{}{}",
            self.scene.instance_count,
            self.scene.mesh_count,
            self.scene.instance_meshlets() as f64 / 1e6,
            self.scene.total_triangles as f64 / 1e6,
